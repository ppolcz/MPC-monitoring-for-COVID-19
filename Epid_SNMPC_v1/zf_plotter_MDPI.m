...
if args.FigNr == 1
    ...
    ... "$1 \times $\,St.dev. of $\hat \beta$ $(\approx 68\%)$", ...
        "$1\sigma$ confidence interval of $\hat \beta$", ...
    ... "$2 \times $\,St.dev. of $\hat \beta$ $(\approx 95\%)$"
        "$2\sigma$ confidence interval of $\hat \beta$", ...
        ],'Location','northwest');
    Leg.NumColumns = 2;
    ...
    Pl_Atlo = plot(r.AtloTeam.d_Rt,r.AtloTeam.Rt_ma7,'Color',Colors.Color_3);
    Sh = zf_plot_mean_var(r.d_val(1:end-1),r.y_val(3,1:r.N_rec)',sqrt(r.Sy_diag(3,1:r.N_rec))');
    title(Titles{Cnt},'Interpreter','latex','FontSize',12)
    grid on

    Leg = legend([Pl_Atlo Sh(1)],[ ...
        "$R_t$ published by Atlo Team [72]", ...
        "Computed $R_t$", ...
        % "$1 \times $\,St.dev. of $\hat \beta$ $(\approx 68\%)$", ...
        % "$2 \times $\,St.dev. of $\hat \beta$ $(\approx 95\%)$"
        ],'Location','northwest');
